...
export default function PatientDecryptPage() {
  ...
  const [decrypted, setDecrypted] = React.useState(data.patient !== null);
  ...
  const handleSearch = async (e) => {
    ...
    if (uuid.length === 0) {
      setError(true);
      setErrorMessage("UUID is required");
      return;
    }
  }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-800">
      ...
      <Form method="post" className="flex flex-col justify-center ">
          <div className="flex flex-col md:flex-row ">
            <label className="text-2xl text-slate-300 my-3 md:mx-3 md:my-0">Enter UUID : </label>
            <input
              type="text"
              name="uuid"
              value={uuid}
